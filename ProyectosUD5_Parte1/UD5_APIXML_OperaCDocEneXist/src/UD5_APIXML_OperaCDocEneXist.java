...
public class UD5_APIXML_OperaCDocEneXist {
	...
	public static void verColecciones() throws Exception { 
		...
		try {
			...
			String URI = "xmldb:exist://localhost:8080/exist/xmlrpc/db/ColeccionesXML", usu = "admin", pwd = "alumno"; 
			col = DatabaseManager.getCollection(URI, usu, pwd);
			
			// Obtiene el n�mero de colecciones en la BD
	...
	public static void verRecursosColecciones() throws Exception { 
		...
		try {
			...
			String URI = "xmldb:exist://localhost:8080/exist/xmlrpc/db/ColeccionesXML", usu = "admin", pwd = "alumno"; 
			col = DatabaseManager.getCollection(URI, usu, pwd);
			// Obtiene el n�mero y lista de colecciones
			System.out.println("N�mero de colecciones: " + col.getChildCollectionCount());
			String[] colecciones = col.listChildCollections();
	...
	public static void crearColeccionyDocumento(String nombreColecc) throws XMLDBException { 
		...
		try {
			...
			String URI = "xmldb:exist://localhost:8080/exist/xmlrpc/db/", usu = "admin", pwd = "alumno";
			col = DatabaseManager.getCollection(URI, usu, pwd);
			
			// CREA UNA NUEVA COLECCION con el nombre pasado como par�metro al m�todo y DENTRO DEL CONTEXTO definido en col
			if (col != null) {
			...
			// Sube un documento a la colecci�n que acaba de crear
			URI = "xmldb:exist://localhost:8080/exist/xmlrpc/db/" + nombreColecc;	// Pone el contexto en la colecci�n creada
			col = DatabaseManager.getCollection(URI, usu, pwd);
			File archivo = new File("NUEVAS_ZONAS.xml");			// El fichero est� ubicado en la carpeta del proyecto
			if (!archivo.canRead()) {
				System.out.println("ERROR AL LEER EL FICHERO");
			} else {
				// Adem�s del nombre del fichero al m�todo createREsource debemos indicarle el tipo de recurso.
				...
				System.out.println("FICHERO A�ADIDO A LA COLECCION " + nombreColecc);
			}
	...
	public static void eliminarDocumento(String colecc, String fichero) throws XMLDBException, Exception {
		...
		try {
			...
			String URI = "xmldb:exist://localhost:8080/exist/xmlrpc/db/" + colecc +"/", usu = "admin", pwd = "alumno";
			System.out.println("URI: " + URI);
			col = DatabaseManager.getCollection(URI, usu, pwd);
			System.out.println("Voy a eliminar el fichero: " + fichero + " de la colecci�n: " + colecc );
